/**
 * NGD-SLAM evaluation entry point for the BONN RGB-D Dynamic dataset.
 *
 * Invocation by run_bonn.sh:
 *   main_bonn <config_yaml> <kernel> <dynamic_filtering> <pangolin_viewer> <package_path>
 *
 * The per-run <config_yaml> contains:
 *   settings_file:          path to camera/ORB settings YAML (e.g. input_data/bonn1200.yaml)
...
{
    if (argc < 2) {
        cerr << "\nUsage: main_bonn <config_yaml> [kernel] [dynamic_filtering]"
                " [pangolin_viewer] [package_path]\n\n";
        return 1;
    }
    ...
    // ── 2. Apply CLI overrides (run_bonn.sh passes these) ──────────────────
    ...
    if (argc >= 6) {
        if (chdir(argv[5]) != 0)
            cerr << "Warning: chdir to package_path failed: " << argv[5] << "\n";
    }

    cout << "\n=== BONN RGB-D evaluation ===\n"
...
// -----------------------------------------------------------------------------
// LoadImages: parse a TUM/BONN-style association file
